...
const ChapterCreateRequestSchema = z
  .object({ projectId: z.string().uuid(), input: CreateChapterInputSchema })
  .strict();
const ChapterUpdateRequestSchema = z
  .object({ chapterId: z.string().uuid(), input: UpdateChapterInputSchema })
  .strict();
const ProviderClearKeyRequestSchema = z
  .object({ providerId: ProviderIdSchema })
  .strict();
const GenerationCreateRequestSchema = z
  .object({
    requestId: z.string().uuid(),
    input: DesktopGenerationInputSchema,
  })
  .strict();
const GenerationCancelRequestSchema = z
  .object({ requestId: z.string().uuid() })
  .strict();
const GenerationIdRequestSchema = z
  .object({ generationId: z.string().uuid() })
  .strict();
const LifecycleResolveCloseRequestSchema = z
  .object({ requestId: z.string().uuid(), canClose: z.boolean() })
  .strict();
...
export interface DesktopDialogAdapter {
  selectImportSource(): Promise<{
    readonly cancelled: boolean;
    readonly sourcePath?: string;
  }>;
  selectExportTarget(): Promise<{
    readonly cancelled: boolean;
...
export function registerDesktopIpcHandlers(
  ...
  registerHandler(
    ...
    EmptyInputSchema,
    () => getWorkspace(dependencies.databaseManager.getRuntime()),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.projectCreate,
    CreateProjectInputSchema.strict(),
    (input) =>
      dependencies.databaseManager.runWrite((runtime) =>
        runtime.workspaceRepository.createProject(input),
      ),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.chapterCreate,
    ChapterCreateRequestSchema,
    ({ projectId, input }) =>
      dependencies.databaseManager.runWrite((runtime) =>
        runtime.workspaceRepository.createChapter(projectId, input),
      ),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.chapterUpdate,
    ChapterUpdateRequestSchema,
    ({ chapterId, input }) =>
      dependencies.databaseManager.runWrite((runtime) =>
        runtime.workspaceRepository.updateChapter(chapterId, input),
      ),
  );
  registerHandler(
    dependencies,
  ...
  registerHandler(
    ...
    ListProviderModelsInputSchema,
    async (input) =>
      (dependencies.providerModelLister ?? listOpenAICompatibleModels)(
        await dependencies.providerVault.resolveModelListing(input),
      ),
  );
  registerHandler(
    dependencies,
    ...
    EmptyInputSchema,
    () => dependencies.providerVault.getSettings(),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.providerSaveSettings,
    SaveProviderSettingsInputSchema.strict(),
    (input) => dependencies.providerVault.saveSettings(input),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.providerClearKey,
    ProviderClearKeyRequestSchema,
    ({ providerId }) => dependencies.providerVault.clearKey(providerId),
  );
  registerHandler(
    dependencies,
    ...
    async ({ requestId, input }) =>
      dependencies.databaseManager.runGeneration(
        await dependencies.providerVault.resolveGeneration(input),
        requestId,
      ),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.generationCancel,
    GenerationCancelRequestSchema,
    ({ requestId }) => {
      dependencies.databaseManager.cancelGeneration(requestId);
      return undefined;
    },
  );
  registerHandler(
    ...
    GenerationIdRequestSchema,
    ({ generationId }) =>
      dependencies.databaseManager.runWrite((runtime) =>
        runtime.generationService.accept(generationId),
      ),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.generationDiscard,
    GenerationIdRequestSchema,
    ({ generationId }) =>
      dependencies.databaseManager.runWrite((runtime) =>
        runtime.generationService.discard(generationId),
      ),
  );
  registerHandler(
    dependencies,
    DESKTOP_CHANNELS.databaseStatus,
    EmptyInputSchema,
    () => dependencies.databaseManager.initialize(),
  );
  registerHandler(
    dependencies,
    ...
    async () => {
      ...
      return {
        cancelled: false,
        workspace: await dependencies.databaseManager.importDatabase(
  ...
  registerHandler(
    ...
    async () => {
      ...
      if (selection.cancelled || !selection.destinationPath) {
        return { cancelled: true };
      }
  ...
  registerHandler(
    ...
    LifecycleResolveCloseRequestSchema,
    (input) => {
...
function registerHandler<T extends z.ZodType>(
  dependencies: DesktopIpcDependencies,
  ...
  dependencies.ipcMain.handle(channel, async (event, input) => {
    ...
    try {
      return success(await action(parsed.data));
    } catch (error) {
...
function validationFailure(error: z.ZodError): DesktopResult<never> {
  ...
  const publicError: ApiError["error"] = {
