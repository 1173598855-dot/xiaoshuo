...
export interface AutoNovelApi {
  ...
  resumeRun(runId: string, provider: ProviderConfig): Promise<ProductionRun>;
...
export function createAutoNovelApi(
  ...
  return {
    ...
    async createBook(input, provider, idempotencyKey) {
      ...
      const parsedProvider = ProviderConfigSchema.parse(provider);
      const body = await requestJson(fetchImpl, "/api/books", {
        method: "POST",
        body: JSON.stringify({
          ...parsedInput,
          provider: parsedProvider,
          idempotencyKey: StartProductionInputSchema.shape.idempotencyKey.parse(
            idempotencyKey,
          ),
        }),
      });
      const schema = z
        .object({ book: BookSchema, directions: z.array(z.unknown()) })
        .strict();
      const parsed = schema.parse(body);
      return {
        book: parsed.book,
        directions: parsed.directions as StoryDirection[],
      };
    },
    async getBook(bookId) {
      return BookDetailsSchema.parse(
        await requestJson(fetchImpl, `/api/books/${bookId}`),
      );
    },
    async selectDirection(bookId, directionId, expectedBookRevision, provider) {
      const body = SelectDirectionInputSchema.merge(
        z.object({ provider: ProviderConfigSchema }).strict(),
      ).parse({ expectedBookRevision, provider });
      return BookDetailsSchema.parse(
        await requestJson(
          fetchImpl,
          `/api/books/${bookId}/directions/${directionId}/select`,
          { method: "POST", body: JSON.stringify(body) },
        ),
      );
    },
    async startProduction(bookId, provider, idempotencyKey) {
      const body = StartProductionInputSchema.merge(
        z.object({ provider: ProviderConfigSchema }).strict(),
      ).parse({ idempotencyKey, provider });
      return ProductionRunSchema.parse(
        await requestJson(fetchImpl, `/api/books/${bookId}/production`, {
          method: "POST",
    ...
    async getRun(runId, signal) {
      return RunDetailsSchema.parse(
        await requestJson(fetchImpl, `/api/production-runs/${runId}`, { signal }),
      );
    },
    async pauseRun(runId) {
      return ProductionRunSchema.parse(
        await requestJson(fetchImpl, `/api/production-runs/${runId}/pause`, {
          method: "POST",
    ...
    async resumeRun(runId, provider) {
      return ProductionRunSchema.parse(
        await requestJson(fetchImpl, `/api/production-runs/${runId}/resume`, {
          method: "POST",
    ...
    async cancelRun(runId) {
      return ProductionRunSchema.parse(
        await requestJson(fetchImpl, `/api/production-runs/${runId}/cancel`, {
          method: "POST",
    ...
    async exportBook(bookId, format) {
      ...
      const result = z
        .object({ format: ExportBookInputSchema.shape.format, content: z.string() })
        .strict()
        .parse(
          await requestJson(fetchImpl, `/api/books/${bookId}/export`, {
            method: "POST",
...
async function requestJson(
  ...
  if (!response.ok) {
    ...
    throw new ApiRequestError(
      ...
      parsed.success ? parsed.data.error.message : "本地服务无法完成请求。",
      parsed.success ? parsed.data.error.fieldErrors : undefined,
    );
  }
  return body;
}
