...
export function ProductionRoom({
  ...
  return (
    <main className="production-page" ref={motionRef}>
      ...
      {taskOpen ? <ProductionTaskPanel bookId={book.book.id} currentRunId={run?.run.id ?? null} api={api} onClose={() => setTaskOpen(false)} onOpenRun={onOpenRun} /> : null}
      <ChapterWorkspace key={book.book.id} bookId={book.book.id} plans={book.chapterPlans} chapters={run?.acceptedChapters ?? []} review={review} tools={<>
