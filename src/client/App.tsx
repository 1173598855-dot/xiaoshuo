...
export function App() {
  ...
  if (page === "home") {
    return <><CreativeHome books={books} busy={busy} error={error} assetDraft={assetDraft} onCreateIdea={(input, autoStart) => { setAssetDraft(null); void createIdea(input, autoStart); }} onOpenBook={(book) => void openBook(book)} onConfigureProvider={() => setProviderOpen(true)} onConfigureWorkflow={() => setWorkflowOpen(true)} onOpenAssetLibrary={() => setAssetOpen(true)} onOpenData={() => setDataOpen(true)} motionMode={motionMode} onToggleMotionMode={() => setMotionMode((current) => current === "quiet" ? "full" : "quiet")} onOpenCommandPalette={() => setCommandOpen(true)} onOpenNavigation={() => setNavigationOpen(true)} onRetry={() => { setLoading(true); setError(null); void loadLibrary(); }} />{assetPanel}{navigationDrawer}{dataDialog}{providerDialog(providers, providerSettings, providerOpen, handleProviderSave, handleProviderClearKey, setProviderOpen)}{workflowDialog()}{commandPalette}</>;
  }
