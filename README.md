...
- 首页聚焦最近作品和“新建故事”入口；有本地草稿时显示字数并可直接继续上次构思。新建故事会进入独立创作页，支持悬疑短篇、都市连载和东方幻想预设，也可以只写自己的想法；方向数量可在 1–12 之间调整，未完成草稿会自动保存并在下次打开时恢复，也可以保存自定义预设；
...
- 2026-09-24 工作台改版让“继续作品 / 新建故事”并列呈现；新建故事现在进入独立创作页，保留更大的故事编辑区与构思提示。方向卡可先预览再选择，生产室默认聚焦当前候选；章节规划、历史、生产日志和作者工具按需展开，往返正文时恢复章节与阅读位置。实现边界与验收记录见[改版设计](docs/superpowers/specs/2026-09-24-workbench-rearchitecture-design.md)和[实施计划](docs/superpowers/plans/2026-09-24-workbench-rearchitecture.md)。
...
- 全源码审查补强限流器、Provider 重试、阶段超时、首页续写和灵感册按需加载；范围与边界记录见 [`2026-09-25-source-audit-optimization.md`](docs/superpowers/plans/2026-09-25-source-audit-optimization.md)。
...
- 2026-09-26 全栈性能优化记录在 [`2026-09-26-startup-performance-optimization.md`](docs/superpowers/plans/2026-09-26-startup-performance-optimization.md)：普通桌面启动复用迁移前的完整性检查结果，历史候选文本补写改为 schema v6 一次性迁移，质量门禁在候选采纳时复用作品详情以缩短 SQLite 写事务。

## 界面方向

前端采用 Xiaoyi Studio 的沉浸式工作台语言：近黑画布、紫色主操作色、细边框和低干扰层级，玻璃材质只用于顶栏、抽屉和临时操作层。首页以最近作品和独立新建入口为主；创作页为故事想法提供更宽的写作画布与构思提示；书架以带封面、书脊和书页厚度的 CSS 立体效果呈现真实作品，支持书架/列表切换。生产室提供章节树、正文/候选审核、章纲和上下文工具组成的三栏工作区，左右栏可收起、调整宽度，专注模式隐藏两侧控制栏并可用 `Escape` 退出。全局工作区导航从顶栏打开左侧抽屉，保留当前页面与作品上下文；页面顶栏复用快捷操作组和状态反馈条，平板/移动端的章节与上下文栏改为可关闭的侧抽屉。模型工作流模式使用带说明、选中态和键盘操作的暗色选择器，多模型协作按规划导演、章节写作、内容审核和问题修复分组展示。方向卡片聚焦后按 `Space` Peek 预览，`Ctrl/Cmd + K` 打开按上下文过滤的快速操作面板；所有主要动画都响应 `prefers-reduced-motion`。桌面端保持高密度工作台，移动端在 `390x844` 下折叠为单列且不产生页面横向滚动。
...
前端动效继续采用“有状态才动”的原则：工作区导航可筛选、快速操作会记录最近使用、独立创作页的灵感册支持一键换灵感、书架提供触摸安全的指针景深、生产室展示 Anime.js 故事脉冲、正文显示阅读进度；作者可以在导航抽屉切换完整/安静动效。五个公开参考库的检索、许可边界和落地清单见 [`2026-09-21-motion-library-upgrade.md`](docs/superpowers/plans/2026-09-21-motion-library-upgrade.md)。
