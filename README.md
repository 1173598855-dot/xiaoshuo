...
前端采用 Xiaoyi Studio 的空间化工作台语言：浅灰画布、石墨文字、蓝色主操作色和白色实体内容面板，借鉴 Apple HIG 的层次与动效原则，同时只把液态玻璃用于顶栏、抽屉和临时操作层。首页把故事想法作为第一视觉主体，方向页用可配置数量的卡片画廊承载候选，生产室用状态、进度和章节记录表达 AI 的阶段性工作。方向卡片聚焦后按 `Space` Peek 预览，`Ctrl/Cmd + K` 打开按上下文过滤的快速操作面板，工作流设置在移动端退化为底部 sheet；所有主要动画都响应 `prefers-reduced-motion`。桌面端保持高密度工作台，移动端在 `390x844` 下折叠为单列且不产生页面横向滚动。
